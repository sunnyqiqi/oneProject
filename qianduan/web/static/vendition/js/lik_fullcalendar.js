...
$(function () {
    ...
    const currentOnlyYearMonthDate = onlyLikDate.getFullYear() + '-' + likDoubleNumber(onlyLikDate.getMonth() + 1) + '-' + likDoubleNumber(onlyLikDate.getDate());
    //get current year
    ...
    function getCurrentMonthDays(curYear, curMon) {
        ...
        for (var i = firstDay; i < daysLength + firstDay; i++) {
            $('#lik-fullcalendar .day-box .lik-date-item').eq(i).addClass('lik-cur-month-day').attr('lik-data-date', curYear + '-' + likDoubleNumber(curMon + 1) + '-' + likDoubleNumber(i + 1 - firstDay));
        }
        //current date light
        ...
        $.each($('#lik-fullcalendar .lik-week-row'), function (i, v) {
            if (!$('#lik-fullcalendar .lik-week-row').eq(i).find('.lik-date-item').hasClass('lik-cur-month-day')) {
                $('#lik-fullcalendar .lik-week-row').eq(i).addClass('lik-none').removeClass('lik-block');
            } else {
                $('#lik-fullcalendar .lik-week-row').eq(i).removeClass('lik-none').addClass('lik-block');
            }
        });
        //control the style
        var likWeekRowShowLen = $('#lik-fullcalendar .lik-week-row.lik-block').length;
        var likWeekDayHeight = ($('#lik-fullcalendar').height() - 70) / likWeekRowShowLen;
        for (var i = 0; i < likWeekRowShowLen; i++) {
            $('#lik-fullcalendar .lik-week-row').css('height', '100px');
            //$('#lik-fullcalendar .lik-week-row').height(likWeekDayHeight);
        }
    }
