$(function () {
    ...
    if(loginUserInfo['company_admin'] != 1){
        var venPayFlowPowerList = loginUserInfo['powerUrls'];
        var venPayFlowAdd = 'customer-pay-flow/add';
        var venPayFlowQyTy = 'customer-pay-flow/status';
        var venPayFlowEdit = 'customer-pay-flow/update';
        var venPayFlowdel = 'customer-pay-flow/del';

        //新建回款流程
        if($.inArray(venPayFlowAdd, venPayFlowPowerList) == -1){
            $('#ven_pay_flow_create').hide();
        }else{
            $('#ven_pay_flow_create').show();
        }

        //启用停用
        var qytyBtnStatus = '';
        if($.inArray(venPayFlowQyTy, venPayFlowPowerList) == -1){
            qytyBtnStatus = 'none';
        }else{
            qytyBtnStatus = '';
        }
        //编辑
        var editBtnStatus = '';
        if($.inArray(venPayFlowEdit, venPayFlowPowerList) == -1){
            editBtnStatus = 'none';
        }else{
            editBtnStatus = '';
        }
        //删除
        var delBtnStatus = '';
        if($.inArray(venPayFlowdel, venPayFlowPowerList) == -1){
            delBtnStatus = 'none';
        }else{
            delBtnStatus = '';
        }
                ...
                if (oE.code == 0) {
                    ...
                    $.each(datalist, function (i, v) {
                        ...
                        if (datalist[i]['status'] == 0) {
                            statusClass = 'c_g';
                            sBtn = '<button class="'+qytyBtnStatus+' but_cancel but_mix but_r lik_pay_flow_btn_ty">停用</button>'
                        } else {
                            statusClass = 'c_r';
                            sBtn = '<button class="'+qytyBtnStatus+' but_cancel but_mix but_lv lik_pay_flow_btn_qy">启用</button><button class="'+editBtnStatus+' but_cancel but_mix but_exit val_dialog ven_pay_flow_edit_btn" name="vendition_addhklc_exit">编辑</button><button class="'+delBtnStatus+' but_cancel but_mix but_r val_dialog lik_pay_flow_btn_del" name="xs_hklc_delete">删除</button>'
                        }
        ...
        $.ajax({
            ...
            success: function (oE) {
                if (oE.code == 0) {
                    ...
                    // 应用部门id和name放在一个数组里
                    var aSearchDept = getJsonArr(getDataArr(sSearchDeptId.slice(0, sSearchDeptId.length - 1), sSearchDeptName.slice(0, sSearchDeptName.length - 1)));
    ...
    // 搜索应用部门
    $('#ven_pay_flow_search_dept_ul li').die('click').live('click', function () {
        payFlowData.dept = $(this).attr('searchdeptchosenid');
        getPayFlowList();
    });
    //搜索创建人
    $('#ven_pay_flow_search_uname_ul li').die('click').live('click', function () {
        payFlowData.creater_id = $(this).attr('searchUid');
        getPayFlowList();
    });
    //搜索负责人
    $('#ven_pay_flow_search_updataname_ul li').die('click').live('click', function () {
        payFlowData.owner_id = $(this).attr('searchUpid');
        getPayFlowList();
    });
    //搜索 状态
    $('#ven_pay_flow_search_status_ul li').die('click').live('click', function () {
        /*if($(this).index() == 0){
    ...
    //不显示作废状态
    $('#ven_pay_flow_noShow').die('click').live('click', function () {
        ...
        $.ajax({
            ...
            success: function (oE) {
                if (oE.code == 0) {
                    ...
                    //状态  更多
                    if (data['status'] == 1) {
                        $('#ven_pay_flow_look_more').html('<li class="'+qytyBtnStatus+' " id="ven_pay_flow_look_more_qy">启用</li><li class="'+editBtnStatus+' val_dialogTop" name="vendition_addhklc_exit" id="ven_pay_flow_look_more_edit">编辑</li><li class="'+delBtnStatus+' val_dialog" id="ven_pay_flow_look_more_del" name="xs_hklc_delete">删除</li>');
                    } else if (data['status'] == 0) {
                        $('#ven_pay_flow_look_more').html('<li class="'+qytyBtnStatus+' " id="ven_pay_flow_look_more_ty">停用</li>');
                    }
                    //基本信息
                    //创建时间
                    ...
                    $.each(data['steps'], function (i, v) {
                        ...
                        sPayStep += '<div class="xs_lc_stephk" name="xs_lc_stephk' + (i + 1) + '" style="display: ' + (i == 0 ? 'block' : 'none') + ';">\
                                            <p class="l-s-x"><h4 style="line-height:12px;font-weight:bold;">阶段' + (i + 1) + '</p></h4><span class="f_color">回款率：' + v['pay_rate'] + '%</span>&nbsp;&nbsp;<span class="f_color m_left_10">周期' + v['segment'] + '天</span><hr class="xs_xslc_hrLine"><span class="f_color">超过' + v['overdue'] + '天扣除提成比例' + v['deduct_rate'] + '%</span>\
                                            <h4 class="" style="line-height:12px;font-weight:bold;">阶段要求</h4>\
                                            <p class="l-s-x" style="margin-bottom:20px;">' + v['content'] + '</p>\
                                            <h4 class="" style="line-height:12px;font-weight:bold;">' + (v['remind'] == 0 ? "不提醒" : "提醒") + '</h4>\
    ...
    //查看>启用
    $('#ven_pay_flow_look_more_qy').die('click').live('click', function () {
        venPayFlowQy(payFlowId)
        $('.slider_head_More').trigger('click');
        $('.slider_head_close').trigger('click');
    });
    //查看>停用
    $('#ven_pay_flow_look_more_ty').die('click').live('click', function () {
        venPayFlowTy(payFlowId);
        $('.slider_head_More').trigger('click');
        $('.slider_head_close').trigger('click');
    });
    ...
    $('#ven_pay_flow_create').die('click').live('click', function () {
        ...
        //新建回款流程创建人
        $('.tanceng .ven_pay_flow_create_uname').html(uname);
        //新建回款流程创建时间
    ...
    function overCheckbox() {
        $.each($('.tanceng .ven_pay_flow_create_steps li'), function (i, v) {
            if ($('.tanceng .ven_pay_flow_create_over_checkbox').attr('checked') == 'checked') {
                $('.tanceng .ven_pay_flow_create_steps li').eq(i).find('input:text').eq(2).val('').attr('disabled', null);
                $('.tanceng .ven_pay_flow_create_steps li').eq(i).find('input:text').eq(3).val('').attr('disabled', null);
            } else {
                $('.tanceng .ven_pay_flow_create_steps li').eq(i).find('input:text').eq(2).val('').attr('disabled', 'disabled');
                $('.tanceng .ven_pay_flow_create_steps li').eq(i).find('input:text').eq(3).val('').attr('disabled', 'disabled');
            }
        })
    }
    ...
    //新建回款流程点击复选框效果
    $('.tanceng .ven_pay_flow_create_steps_content').find('input:checkbox').die('click').live('click', function () {
    ...
    $('.tanceng .ven_pay_flow_create_over_checkbox').die('click').live('click', function () {
        overCheckbox();
    });
    ...
    function venPayFlowChooseDept() {
     ...
     success: function (oE) {
                ...
                //选择部门左侧选择
                $('.tanceng .ven_pay_flow_dept_list ul .deptChild').die('click').die('click).live('click', function () {
                ...
                //新建>选择部门确认
                $('.tanceng .ven_pay_flow_create_choose_dept_save').die('click').die('click).live('click', function () {
    ...
    $('.tanceng .ven_pay_flow_create_save').die('click').live('click', function () {
        ...
        $.each($('.tanceng .ven_pay_flow_create_steps li'), function (i, v) {
            ...
            //判断回款率总和
            venPayFlowHkl += parseFloat($('.tanceng .ven_pay_flow_create_steps li').eq(i).find('input:text').eq(0).val());
            //判断超出周期扣除提成金额比例是否选中
        ...
        console.log(parseFloat(venPayFlowHkl));
        if (parseFloat(venPayFlowHkl) > 100) {
            alert('回款率总和不能超过100%');
            return false;
        }else if(parseFloat(venPayFlowHkl) < 100){
            alert('回款率总和不足100%，请完善');
            return false;
        }
        //选择部门
        ...
        venPayFlowCreateData.dept_list = venPayFlowCreateData.dept_list.slice(0, venPayFlowCreateData.dept_list.length);
    ...
    //查看>编辑
    $('#ven_pay_flow_look_more_edit').die('click').live('click', function(){
        $('.slider_head_More').trigger('click');
        $('.slider_head_close').trigger('click');
        venPayFlowEditData = {
            token: token,
    ...
    $('.tanceng .ven_pay_flow_edit_over_checkbox').die('click').live('click', function () {
        overCheckboxEdit()
    })
    ...
    function getPayFlowInfoEdit(payFlowId) {
        $.ajax({
            ...
            success: function (oE) {
                ...
                if (oE.code == 0) {
                    //回款流程创建人
                    $('.tanceng .ven_pay_flow_edit_uname').html(data['uname']);
                    //回款流程创建日期
                    ...
                    if(data['steps'].length == 0){
                        venPayFlowEditStepsList = '<li><div><span class="xslc_setul_1">阶段<cite>1</cite></span><div class="xslc_setul_5"><input type="text" onfocus="fn_focus(this);" onblur="fn_blur(this);" onkeydown="value=value.replace(/[^\-?\\d.]/g,\'\')" onkeyup="value=value.replace(/[^\-?\\d.]/g,\'\')"><i>%</i></div><div class="xslc_setul_5"><input type="text" onfocus="fn_focus(this);" onblur="fn_blur(this);" onkeydown="value=value.replace(/[^\-?\\d.]/g,\'\')" onkeyup="value=value.replace(/[^\-?\\d.]/g,\'\')"><i>天</i></div><div class="xslc_setul_5"><input type="text" onfocus="fn_focus(this);" onblur="fn_blur(this);" onkeydown="value=value.replace(/[^\-?\\d.]/g,\'\')" onkeyup="value=value.replace(/[^\-?\\d.]/g,\'\')"><i>天</i></div><div class="xslc_setul_5"><input type="text" onfocus="fn_focus(this);" onblur="fn_blur(this); "onkeydown="value=value.replace(/[^\-?\\d.]/g,\'\');value=value>100?(value.slice(0,value.length-1)):value" onkeyup="value=value.replace(/[^\-?\\d.]/g,\'\');value=value>100?(value.slice(0,value.length-1)):value"><i>%</i></div><div class="xslc_setul_4"><button class="hklc_setulAddli but_green">+</button><button class="hklc_setuldelli but_red">-</button></div></div></li>';
                        ...
                        $.each(data['steps'], function (i, v) {
                            venPayFlowEditStepsList += '<li><div><span class="xslc_setul_1">阶段<cite>' + (i + 1) + '</cite></span><div class="xslc_setul_5"><input class="c_3" type="text" value="' + v['pay_rate'] + '" onkeydown="value=value.replace(/[^\-?\\d.]/g,\'\')" onkeyup="value=value.replace(/[^\-?\\d.]/g,\'\')"><i>%</i></div><div class="xslc_setul_5"><input class="c_3" type="text" value="' + v['segment'] + '" onkeydown="value=value.replace(/[^\-?\\d.]/g,\'\')" onkeyup="value=value.replace(/[^\-?\\d.]/g,\'\')"><i>天</i></div><div class="xslc_setul_5"><input class="c_3" type="text" value="' + v['overdue'] + '" onkeydown="value=value.replace(/[^\-?\\d.]/g,\'\')" onkeyup="value=value.replace(/[^\-?\\d.]/g,\'\')"><i>天</i></div><div class="xslc_setul_5"><input class="c_3" type="text" value="' + v['deduct_rate'] + '" onkeydown="value=value.replace(/[^\-?\\d.]/g,\'\');value=value>100?(value.slice(0,value.length-1)):value" onkeyup="value=value.replace(/[^\-?\\d.]/g,\'\');value=value>100?(value.slice(0,value.length-1)):value"><i>%</i></div><div class="xslc_setul_4"><button class="hklc_setulAddli but_green">+</button><button class="hklc_setuldelli but_red">-</button></div></div></li>'
                            venPayFlowEditStepsContents += '<div>\
                                                            <div class="t_textinput t_textarea">\
                                                            <div class="t_left"><i class="c_r">*</i>阶段<cite>' + (i + 1) + '</cite>要求</div>\
                                                            <div class="t_right"><textarea class="txt_normal c_3 ven_pay_flow_edit_steps_content_textarea">' + v['content'] + '</textarea></div>\
                                                        </div>\
                                                        <div class="vendition_xslcss"><input type="checkbox" class="ven_pay_flow_edit_remind_checkbox" ' + (v['remind'] == "1" ? "checked" : "") + '/><span>本阶段提前</span><input class="c_3 inp_dis_mid" type="text" ' + (v['remind'] == "1" ? "" : "disabled") + ' value="' + (v['remind'] == "1" ? (v["days"]) : "") + '"/><span>天时提醒销售人员执行</span>\
                                                            </div>\
                                                        </div>'
                        });
                    }
                    ...
                    $.each($('.tanceng .ven_pay_flow_edit_steps_content .ven_pay_flow_edit_remind_checkbox'), function (i, v) {
                        $('.tanceng .ven_pay_flow_edit_steps_content .ven_pay_flow_edit_remind_checkbox').die('click').live('click', function () {
                            if ($(this).attr('checked') == 'checked') {
                                $(this).siblings('input:text').attr('disabled', null);
                            } else {
        ...
        $.ajax({
            ...
            success: function (oE) {
                ...
                for (var i = 0, liLeft1 = $('.tanceng .ven_pay_flow_dept_list li.left_1').length; i < liLeft1; i++) {
                    ...
                    $('.tanceng .ven_pay_flow_dept_list li.left_1').eq(i).addClass('deptChild');
                }
                ...
                $('.tanceng .left_all').die().live('click', function () {
                    $(this).toggle(function () {
                        ...
                        var listhtmlall = '';
                        $('.tanceng .ven_pay_flow_dept_list').find('li.deptChild').each(function (i, v) {
                            listhtmlall += '<li uid="' + $(this).attr('deptchosenid') + '"><span>' + $(this).children('.list_msg').html() + '</span><i class="list_choose_delete"></i></li>';
                            $(this).find('span.list_check em').addClass('on');
                        })
                        $('.tanceng .ven_pay_flow_dept_chosen').html(listhtmlall)
                        $('.tanceng .ven_pay_flow_dept_list').css('display', 'none')
                    }, function () {
                        $(this).find('em').removeClass('on');
                        $('.tanceng .ven_pay_flow_dept_chosen').html('')
                        $('.tanceng .ven_pay_flow_dept_list').css('display', 'block')
                        $('.tanceng .ven_pay_flow_dept_list').find('li.deptChild').each(function (i, v) {
                            $(this).find('span.list_check em').removeClass('on');
                        })
                    })
                ...
                $('.tanceng .ven_pay_flow_dept_list ul .deptChild').die('click').live('click', function () {
                    if ($(this).find('em').hasClass('on')) {
                        if($(this).parent('ul').children('ul').length==0){
                            $('.tanceng .ven_pay_flow_dept_chosen').find('li[uid=' + $(this).attr('deptchosenid') + ']').remove();
                            $(this).find('span.list_check em').removeClass('on');
                        }else{
                            var arr = [];
                            $('.tanceng .ven_pay_flow_dept_chosen').find('li[uid=' + $(this).attr('deptchosenid') + ']').remove();
                            ...
                            $('.tanceng .ven_pay_flow_dept_chosen').children().each(function(i,v){
                        ...
                        if($(this).parent('ul').children('ul').length==0){
                            $('.tanceng .ven_pay_flow_dept_chosen').append('<li uid="' + $(this).attr('deptchosenid') + '"><span>' + $(this).children('.list_msg').html() + '</span><i class="list_choose_delete"></i></li>');
                            $(this).find('span.list_check em').addClass('on');
                        }else{
                            ...
                            $('.tanceng .ven_pay_flow_dept_chosen').append(zjlist+listhtml)
                        }

                    }
                    $('.tanceng .ven_pay_flow_dept_list ul li').removeClass('on');
                ...
                $('i.list_choose_delete').die('click').live('click', function () {
                    $(this).closest('li').remove();
                    $('.tanceng .ven_pay_flow_dept_list ul .deptChild[deptchosenid = "' + $(this).closest('li').attr('uid') + '"]').find('em').removeClass('on');
                });
                //新建>选择部门确认
                $('.tanceng .ven_pay_flow_create_choose_dept_save').die('click').live('click', function(){
                    ...
                    var deptChosen = '';
                    $.each($('.tanceng .ven_pay_flow_dept_chosen').find('li'), function(i, v){
                        deptChosen += '<li deptchosenid="'+ $('.tanceng .ven_pay_flow_dept_chosen').find('li').eq(i).attr('uid') +'">'+$('.tanceng .ven_pay_flow_dept_chosen').find('li').eq(i).text()+' <i></i></li>'
                        /*venPayFlowCreateData.dept_list +=$('.tanceng .ven_pay_flow_dept_chosen').find('li').eq(i).attr('uid') + ','
                        venPayFlowEditData.dept_list +=$('.tanceng .ven_pay_flow_dept_chosen').find('li').eq(i).attr('uid') + ','*/
                    });
    ...
    //编辑回款流程 > 选择部门
    $('.tanceng .ven_pay_flow_edit_choose_dept').die('click').live('click', function(){
        venPayFlowChooseDept()
    })
    //编辑回款流程 > 提交
    $('.tanceng .ven_pay_flow_edit_submit').die('click').live('click', function(){
        //编辑回款流程名称
        ...
        venPayFlowEditData.dept_list = venPayFlowEditData.dept_list.slice(0, venPayFlowEditData.dept_list.length);
