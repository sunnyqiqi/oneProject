$(function () {
    ...
    // 切换
    $('#page_105_tab li').die('click').live('click',function(){
        if($(this).index()==0){
            $('.page_105_btn').removeClass('none');
            $('.page_107_btn').addClass('none');
            $('.page_105_div').removeClass('none');
            $('.page_107_div').addClass('none');
            $('.page_107_btn_res').addClass('none1');
            $('.page_105_btn_res').removeClass('none1');
            getDaliyIncomeList();
        }else if($(this).index()==1){
            $('.page_105_btn').addClass('none');
            $('.page_107_btn').removeClass('none');
            $('.page_105_div').addClass('none');
            $('.page_107_div').removeClass('none');
            $('.page_107_btn_res').removeClass('none1');
            $('.page_105_btn_res').addClass('none1');
            getFncExpendList();
        }
    });
    ...
    if (loginUserInfo['company_admin'] != 1) {
        var fncIncomePowerList = loginUserInfo['powerUrls'];
        //新建一笔收入
        var fncIncomeAdd = 'income-expend/add';
        //作废
        var fncIncomeInvalid = 'income-expend/invalid';

        //新建一笔收入
        if ($.inArray(fncIncomeAdd, fncIncomePowerList) == -1) {
            $('#fnc_income_create_btn').hide();
            $('#fnc_income_create_nav').css('width', '52px');
        } else {
            $('#fnc_income_create_btn').show();
            $('#fnc_income_create_nav').css('width', '136px');
        }

        //作废
        var fncIncomeInvalidBtnClass = '';
        if ($.inArray(fncIncomeInvalid, fncIncomePowerList) == -1) {
            fncIncomeInvalidBtnClass = 'none';
        } else {
            fncIncomeInvalidBtnClass = '';
        }

    }
    // 定义选择查看项
    var fncIncomeListLookAbledField = [
        {'index': null, 'field': '结算账户'},
        {'index': null, 'field': '结算账目'},
        {'index': null, 'field': '收款方式'},
        {'index': null, 'field': '备注'}
    ];
    likShow('#fnc_income_table', fncIncomeListLookAbledField, '#fnc_income_look_field_ul', '#fnc_income_look_save', '#fnc_income_look_reset');
    var DailyIncomeData = {
        token: token,
        page: 1,
        num: 10,
    ...
    function getDaliyIncomeList() {
        $.ajax({
            ...
            success: function (oE) {
                ...
                if (oE.code == 0) {
                    //总支出
                    $('.fnc_expend_income_total_money').html(oE.sum_income_total_money);
                    //搜索总条数
                    $(".page_105_search_allNum").html(oE.totalcount);
                    var datalist = oE.datalist;
                    if (datalist.length == 0) {
                        $(".page_105_nodata").removeClass('none');
                        $(".page_105_nodata_fenye").addClass('none');
                    } else {
                        $(".page_105_nodata").addClass('none');
                        $(".page_105_nodata_fenye").removeClass('none');
                    }
                    //字符串拼接
                    var DailyIncomeHtml = '';
                    $.each(datalist, function (i, v) {
                        ...
                        if (v['is_invalid'] == 0) {
                            ...
                            voidStatusClass = '';
                            voidStatusBtn = '<button class="' + fncIncomeInvalidBtnClass + ' but_mix but_r but_void page_105_invalid_btn">作废</button>';
                        } else {
                        ...
                        DailyIncomeHtml += '<tr incomeid="' + v['id'] + '" class="' + voidStatusClass + '">' +
                            '<td>' + voidStatus + '</td>' +
                            '<td>' + v['code_sn'] + '</td>' +
                            '<td>' + incomType + '</td>' +
                            ...
                            '<td>' + incomeWay + '</td>' +
                            '<td>' + v['note'] + '</td>' +
                            '<td><button class="but_mix but_look val_dialog page_105_look_but" name="finance_rcsr_dayin">查看收款单</button>' + voidStatusBtn + '</td>' +
                            '</tr>';
                        // }
                    });
                    // 页面表格数据渲染
                    $(".page_105_income_table").html(DailyIncomeHtml);
                    list_table_render_pagination('.page_105_table_fenye', DailyIncomeData, getDaliyIncomeList, oE.totalcount, datalist.length)
                    $('#fnc_income_look_save').trigger('click');
                }
            }
        })
    }

    var DaliyIncomelookId = null;
    //查看详情
    $(".page_105_look_but").die('click').live('click', function () {
        DaliyIncomelookId = $(this).closest("tr").attr('incomeid');
        $.ajax({
            url: SERVER_URL + '/income-expend/info',
            type: 'GET',
            data: {
                token: token,
                id: DaliyIncomelookId
            },
            dataType: 'json',
            success: function (oE) {
                if (oE.code == 0) {
    ...
    //作废操作
    $(".page_105_invalid_btn").die('click').live("click", function () {
        DaliyIncomelookId = $(this).closest('tr').attr('incomeid');
        $.ajax({
    ...
    //不显示作废状态
    $('.fnc_income_list_no_show').die('click').live('click', function () {
        if ($(this).attr('checked') == 'checked') {
            DailyIncomeData.is_invalid = 0;
        } else {
            DailyIncomeData.is_invalid = '';
        }
        getDaliyIncomeList();
    });
    //搜索关键字
    $('.fnc_income_list_search_btn').die('click').live('click', function () {
        if ($('.fnc_income_list_search_inp').val() == '搜索收入名称') {
            alert('请输入搜索关键字');
            DailyIncomeData.keywords = '';
        } else {
            DailyIncomeData.keywords = $('.fnc_income_list_search_inp').val();
        }
        getDaliyIncomeList();
    });
    //刷新
    $('#fnc_income_refresh_btn').die('click').live('click', function () {
        DailyIncomeData = {
            token: token,
            page: 1,
            num: 10,
        ...
        $('.fnc_income_list_search_inp').val('搜索收入名称').css('color', '#CCCCCC');
        $('.fnc_income_list_search_num').val('10');
        $('.fnc_income_list_no_show').attr('checked', 'checked');
        getDaliyIncomeList();
    });
    //定义新建日常收入参数
    var fncIncomeCreateData = {
        token: token,
        id: 0,
        thetype: 1, // 1收入2支出
        code_sn: '', // 编号
    ...
    $('#fnc_income_create_btn').die('click').live('click', function () {
        fncIncomeCreateData = {
            token: token,
            id: 0,
            thetype: 1, // 1收入2支出
        ...
        //收款人
        $('.tanceng .fnc_income_create_uname').html(uname);
        //收款编号
        $('.tanceng .fnc_income_create_code_sn').val(likGetCodeFn('RCSR'));
    });
    //收入类型
    $('.tanceng .fnc_income_create_type_ul li').die('click').live('click', function () {
        fncIncomeCreateData.company_type = $(this).index() + 1;
    });
    //选择账户
    var companyAccountId = null;
    $('.tanceng .fnc_income_create_choice_account_inp').die('click').live('click', function () {
        $.ajax({
            ...
            success: function (oE) {
                ...
                if (oE.code == 0) {
                    ...
                    $(".tanceng .fnc_income_create_choose_company_ul").html(AccountcompanyNameHTML);
                }
            }
        });
    });
    $(".tanceng .fnc_income_create_choose_company_ul li").die('click').live('click', function () {
        fncIncomeCreateData.choice_account = $(this).attr('companyid');
        companyAccountId = $(this).attr('companyid');
    });
    //选择账目
    $('.tanceng .fnc_income_create_choice_cuenta_inp').die('click').live('click', function () {
        if ($('.tanceng .fnc_income_create_choice_account_inp').val() == '请选择账户') {
            alert('请先选择账户');
            return false;
        } else {
            $.ajax({
                ...
                success: function (oE) {
                    ...
                    if (oE.code == 0) {
                        var datalist = oE.datalist;
                        console.log(datalist);
                        var fncIncomeCreateChooseCuentaList = '';
                        $.each(datalist, function (i, v) {
                            if(v['type'] != 1) return true;
                            fncIncomeCreateChooseCuentaList += '<li accountid="' + v['id'] + '">' + v['name'] + '</li>'
                        });
                        $(".tanceng .fnc_income_create_choice_cuenta_ul").html(fncIncomeCreateChooseCuentaList);
                    }
                }
            });
        }
    });
    $(".tanceng .fnc_income_create_choice_cuenta_ul li").die('click').live('click', function () {
        fncIncomeCreateData.choice_cuenta = $(this).attr('accountid');
    });
    //收款方式
    $('.tanceng .fnc_income_create_choice_way_ul li').die('click').live('click', function () {
        fncIncomeCreateData.way = $(this).index() + 1;
    });
    //收款负责人
    function finIncomCreateChoiceOwnerFn() {
        $.ajax({
            ...
            success: function (oE) {
                if (oE.code == 0) {
                    ...
                    var deep = 0;
                    //负责人列表
                    $('.tanceng .fin_income_create_chioce_owner_list').html(tree_list_person(datalist, deep));
                    //判断部门图标样式
    ...
    //选择收入负责人
    $('.tanceng .fnc_income_create_choice_owner').die('click').live('click', function () {
        finIncomCreateChoiceOwnerFn();
        //选择收入负责人确认
        $('.tanceng .fin_income_create_chioce_owner_save').die('click').live('click', function () {
            fncIncomeCreateData.owner = $('.tanceng .list_check').closest('li').attr('userinfoid');
            $('.tanceng .fin_income_create_choice_owner_inp').val($('.tanceng .list_check').closest('li').find('.list_msg').html()).css('color', '#333');
            $(this).closest('.dialog_box').remove();
        });
    });
    //新建日常收入 - 提交
    $('.tanceng .fnc_income_create_submit_btn').die('click').live('click', function () {
        //收款编号
        fncIncomeCreateData.code_sn = $('.tanceng .fnc_income_create_code_sn').val();
        //收入类型
        if ($('.tanceng .fnc_income_create_type_inp').val() == '请选择收入类型') {
            alert('请选择收入类型');
            return false;
        }
        //公司名称
        if ($('.tanceng .fnc_income_create_company_name_inp').val() == '请输入公司名称') {
            alert('请输入公司名称');
            return false;
        } else {
            fncIncomeCreateData.company_name = $('.tanceng .fnc_income_create_company_name_inp').val();
        }
        //收入名称
        if ($('.tanceng .fnc_income_create_name_inp').val() == '请输入收入名称') {
            alert('请输入收入名称');
            return false;
        } else {
            fncIncomeCreateData.name = $('.tanceng .fnc_income_create_name_inp').val();
        }
        //收入日期
        if ($('.tanceng .fnc_income_create_day_inp').val() == '请选择日期') {
            alert('请选择收入日期');
            return false;
        } else {
            fncIncomeCreateData.day = $('.tanceng .fnc_income_create_day_inp').val();
        }
        //选择账户
        if ($('.tanceng .fnc_income_create_choice_account_inp').val() == '请选择账户') {
            alert('请选择账户');
        ...
        //选择账户
        if ($('.tanceng .fnc_income_create_choice_cuenta_inp').val() == '请选择账目') {
            alert('请选择账目');
            return false;
        }
        //选择收款方式
        if ($('.tanceng .fnc_income_create_choice_way_inp').val() == '请选择收款方式') {
            alert('请选择收款方式');
            return false;
        }
        //支票号
        fncIncomeCreateData.cheque_num = '';
        if ($('.tanceng .fnc_income_create_choice_way_inp').val() == '支票' && $('.tanceng .finance_rcsr_one_check_inp').val() == '请输入支票号') {
            alert('请输入支票号');
            return false;
        } else if ($('.tanceng .fnc_income_create_choice_way_inp').val() == '支票' && $('.tanceng .finance_rcsr_one_check_inp').val() != '请输入支票号') {
            fncIncomeCreateData.cheque_num = $('.tanceng .finance_rcsr_one_check_inp').val();
        }
        //选择收入负责人
        if ($('.tanceng .fin_income_create_choice_owner_inp').val() == '请选择收入负责人') {
            alert('请选择负责人');
            return false;
        }
        //金额
        if ($('.tanceng .fnc_income_create_money_inp').val() == '请输入金额') {
            alert('请输入金额');
            return false;
        } else {
            fncIncomeCreateData.money = $('.tanceng .fnc_income_create_money_inp').val();
        }
        //备注
        if ($('.tanceng .fnc_income_create_note_textarea').val() == '请输入备注内容') {
            fncIncomeCreateData.note = '';
        } else {
            fncIncomeCreateData.note = $('.tanceng .fnc_income_create_note_textarea').val();
        }
        console.log(fncIncomeCreateData);
        $.ajax({
            url: SERVER_URL + '/income-expend/add',
            type: 'POST',
            data: fncIncomeCreateData,
            dataType: 'json',
            success: function (oE) {
                console.log(oE);
                if (oE.code == 0) {
...
//***********日常支出*****************************************************************************************************************
    ...
    function getFncExpendList() {
        $.ajax({
            ...
            success: function (oE) {
                ...
                if (oE.code == 0) {
                    //总支出
                    $('.fnc_expend_expend_total_money').html(oE.sum_expend_total_money);
                    //搜索总条数
    ...
    $('.tanceng .fnc_expend_create_choice_owner').die('click').live('click',function(){
        ...
        $('.tanceng .fin_expend_create_chioce_owner_save').die('click').live('click',function(){
            ...
            $('.tanceng .fin_expend_create_choice_owner_inp').val($('.tanceng .list_check').closest('li').find('.list_msg').html()).css('color', '#333');
            $(this).closest('.dialog_box').remove();
        });
    });
    //收款方式
