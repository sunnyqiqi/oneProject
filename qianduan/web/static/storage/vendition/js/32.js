...
$(function () {
                ...
                if (oE.code == 0) {
                    ...
                    $.each(datalist, function (i, v) {
                        ...
                        if (datalist[i]['status'] == 0) {
                            statusClass = 'c_g';
                            sBtn = '<button class="but_cancel but_mix but_r lik_sell_flow_btn_ty">停用</button>'
                        } else {
                            statusClass = 'c_r';
                            sBtn = '<button class="but_mix but_exit val_dialogTop ven_sell_flow_edit" name="vendition_addxslc_exit">编辑</button><button class="but_cancel but_mix but_lv lik_sell_flow_btn_qy">启用</button><button class="but_mix but_r val_dialog lik_sell_flow_btn_del" name="xs_xslc_delete">删除</button>'
                        }
    ...
    //刷新列表
    $('#ven_sell_flow_refresh').live('click', function () {
        sellFlowData = {
            token: token,
            page: 1, // 页面
            num: 10, // 每页条数
    ...
    // 销售流程查看全部
    $('#ven_sell_flow_searAll').live('click', function () {
        sellFlowData.status = '';
        getSellFlowList();
    });
    //销售流程查看使用中
    $('#ven_sell_flow_searUse').live('click', function () {
        sellFlowData.status = 0;
        getSellFlowList();
    });
    //销售流程查看已停用
    $('#ven_sell_flow_searStop').live('click', function () {
        sellFlowData.status = 1;
        getSellFlowList();
    });
    //搜索关键字
    $('#ven_sell_flow_searKey_btn').live('click', function () {
    ...
    // 搜索应用部门
    $('#ven_sell_flow_search_dept_ul li').live('click', function () {
        sellFlowData.dept = $(this).attr('searchdeptid');
        console.log(sellFlowData.dept);
        getSellFlowList();
    });
    //搜索创建人
    $('#ven_sell_flow_search_uname_ul li').live('click', function () {
        sellFlowData.creater_id = $(this).attr('searchUid');
        getSellFlowList();
    });
    //搜索最后修改人
    $('#ven_sell_flow_search_updataname_ul li').live('click', function () {
        sellFlowData.update_id = $(this).attr('searchUpid');
        getSellFlowList();
    });
    //搜索 状态
    $('#ven_sell_flow_search_status_ul li').live('click', function () {
        /*if($(this).index() == 0){
    ...
    //不显示作废状态
    $('#ven_sell_flow_noShow').live('click', function () {
    ...
    //新建销售流程
    $('#ven_sell_flow_create').live('click', function () {
        //获取创建时系统时间
        $('.tanceng .ven_sell_flow_create_date').html(getCurrentDate());
    });
    //新建销售流程保存
    $('.tanceng .ven_sell_flow_create_save').live('click', function () {
        //判断销售流程名称是否输入
    ...
    //添加部门
    $('.tanceng .ven_sell_flow_choose_dept').live('click', function () {
            ...
            success: function (e) {
                ...
                for (var i = 0, liLeft1 = $('.tanceng .ven_sell_flow_dept_list li.left_1').length; i < liLeft1; i++) {
                    if ($('.tanceng .ven_sell_flow_dept_list li.left_1').eq(i).next('ul').length == 0) {
                        ...
                        $('.tanceng .ven_sell_flow_dept_list li.left_1').eq(i).find('span.personOth').closest('li').addClass('deptChild')
                    }
                }
                //选择部门左侧选择
                ...
                $('.tanceng .ven_sell_flow_create_choose_dept_save').die('click').live('click', function () {
                    ...
                    $.each($('.tanceng .ven_sell_flow_dept_chosen').find('li'), function (i, v) {
                        deptChosen += '<li deptid="' + $('.tanceng .ven_sell_flow_dept_chosen').find('li').eq(i).attr('rid') + '">' + $('.tanceng .ven_sell_flow_dept_chosen').find('li').eq(i).text() + ' <i></i></li>';
                        venSellFlowCreateData.dept_list += $('.tanceng .ven_sell_flow_dept_chosen').find('li').eq(i).attr('rid') + ','
                    });
    ...
    //编辑按钮点击事件
    $('.ven_sell_flow_edit').live('click', function () {
                ...
                if (oE.code == 0) {
                    ...
                    if(data['steps'].length == 0){
                        ...
                        venSellFlowEditStepsContent='<div style="z-index: 1;">\
                            ...
                            <div class="t_right" style="z-index: 2;"><textarea class="txt_normal ven_sell_flow_create_steps_content_textarea" onfocus="fn_focus(this);" onblur="fn_blur(this);">请输入销售流程名称</textarea></div>\
                            </div>\
...
onblur="fn_blur(this);" class="ven_sell_flow_create_steps_content_remind_day"><span>天时提醒销售人员执行</span>\
                            </div>\
                            <div class="vendition_xslcss">\
                            <input type="checkbox" class="ven_sell_flow_create_steps_content_confirm_checkbox"><span>本阶段完成需直属上级确认</span>\
                            </div>\
                            </div>'
                    }else{
                        $.each(data['steps'], function (i, v) {
                            venSellFlowEditSteps += '<li><div><span class="xslc_setul_1">阶段<cite>' + (i + 1) + '</cite></span><div class="xslc_setul_2"><input type="text" value="' + v['name'] + '" onfocus="fn_focus(this);" onblur="fn_blur(this);" class="ven_sell_flow_edit_steps_name"/></div><div class="xslc_setul_3"><input type="text" value="' + v['win_rate'] + '" onfocus="fn_focus(this);" onblur="fn_blur(this);" onkeydown="value=value.replace(/[^\-?\\d.]/g,\'\')" onkeyup="value=value.replace(/[^\-?\\d.]/g,\'\')" class="ven_sell_flow_edit_steps_winrate"/><i>%</i></div><div class="xslc_setul_4"><button class="xslc_edit_setulAddli but_green">+</button><button class="xslc_edit_setuldelli but_red">-</button></div></div></li>';
                            venSellFlowEditStepsContent += '<div><div class="t_textinput t_textarea">\
                            <div class="t_left"><i class="c_r">*</i>阶段<cite>' + (i + 1) + '</cite>要求</div>\
                            <div class="t_right"><textarea class="txt_normal ven_sell_flow_edit_steps_content_textarea" onfocus="fn_focus(this);" onblur="fn_blur(this);">' + v['content'] + '</textarea></div>\
                            </div>\
                            <div class="vendition_xslcss">\
                            <input class="ven_sell_flow_edit_steps_content_remind_checkbox" type="checkbox" ' + (v['remind'] == "1" ? "checked" : "") + '/><span>本阶段停留</span><input class="ven_sell_flow_edit_steps_content_remind_day" value="' + v['days'] + '" type="text" onfocus="fn_focus(this);" onblur="fn_blur(this);"/><span>天时提醒销售人员执行</span>\
                            </div>\
                            <div class="vendition_xslcss">\
                            <input class="ven_sell_flow_edit_steps_content_confirm_checkbox" type="checkbox" ' + (v['remind'] == "1" ? "checked" : "") + '/><span>本阶段完成需直属上级确认</span>\
                            </div></div>'
                        });
                    }
                    ...
                    $.each(venSellFlowEditDeptChosenArr, function (i, v) {
                        venSellFlowEditDeptChosen += '<li deptid="' + v['title'] + '">' + v['val'] + ' <i></i></li>'
                    });
                    ...
                    //选择部门
                    $('.tanceng .ven_sell_flow_edit_choose_dept').live('click', function () {
                        venSellFlowChooseDept();
                    })
                } else {
                    alert('操作失败');
        ...
        $.ajax({
            ...
            success: function (e) {
                ...
                if (oE.code == 0) {
                    ...
                    //状态  更多
                    if (data['status'] == 1) {
                        $('#ven_sell_flow_look_more').html('<li id="ven_sell_flow_look_more_qy">启用</li><li class="val_dialogTop" name="vendition_addxslc_exit" id="ven_sell_flow_look_more_edit">编辑</li><li id="ven_sell_flow_look_more_del" class="val_dialog" name="xs_xslc_delete" id="ven_sell_flow_look_more_del">删除</li>');
                    } else if (data['status'] == 0) {
                        $('#ven_sell_flow_look_more').html('<li id="ven_sell_flow_look_more_ty">停用</li>');
                    }
                    //基本信息
                    //创建时间
    ...
    //查看>更多>启用
    $('#ven_sell_flow_look_more_qy').live('click', function () {
        $.ajax({
            url: SERVER_URL + '/customer-flow/setstatus',
            type: 'GET',
            data: {
    ...
    //查看>更多>停用
    $('#ven_sell_flow_look_more_ty').live('click', function () {
        $.ajax({
            url: SERVER_URL + '/customer-flow/setstatus',
            type: 'GET',
            data: {
    ...
    //查看>更多>删除
    $('#ven_sell_flow_look_more_del').live('click', function () {
    ...
    //查看>编辑
    $('#ven_sell_flow_look_more_edit').live('click', function () {
