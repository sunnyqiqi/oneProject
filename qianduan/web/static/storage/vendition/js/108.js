...
$(function () {
    ...
    //刷新列表
    $('#ven_after_order_refresh').live('click', function () {
        afterOrderData = {
            token: token,
            page: 1, //页面
            num: 10, //每页条数
    ...
    // 售后单查看我发起的
    $('#ven_after_order_searMine').live('click', function () {
    ...
    // 售后单查看我执行的
    $('#ven_after_order_searIdo').live('click', function () {
    ...
    $('#ven_after_order_allot_btn').live('click', function () {
        $('.ven_after_order_search_div').css('display', 'none');
        $('button.ven_after_order_search_btn').css('display', 'none');
        afterOrderAllotFn();
    });
    ...
    //搜索关键字
    $('#ven_after_order_search_btn').live('click', function () {
    ...
    //不显示完成售后
    $('#ven_after_order_notshowok_btn').live('click', function () {
    ...
    //不显示作废
    $('#ven_after_order_notshowinvalid_btn').live('click', function () {
        if ($(this).is(':checked')) {
    ...
    // 搜索客户名称
    $('#ven_after_order_search_cus_name_ul li').live('click', function () {
        afterOrderData.key = $(this).html();
        getAfterOrderList();
    });
    // 搜索拜访类型
    $('#ven_after_order_search_thetype_ul li').live('click', function () {
        afterOrderData.thetype = $(this).index();
        getAfterOrderList();
    });
    //搜索负责人
    $('.ven_after_order_search_owner_ul li').live('click', function () {
        afterOrderData.owner = $(this).attr('ownerid');
        getAfterOrderList();
    });
    //搜索售后类型
    $('.ven_after_order_search_service_type_ul li').live('click', function () {
        afterOrderData.service_type = $(this).index();
        getAfterOrderList();
    });
    //搜索创建人
    $('.ven_after_order_search_uname_ul li').live('click', function () {
        afterOrderData.uid = $(this).attr('uid');
        getAfterOrderList();
    });
    //搜索状态
    $('.ven_after_order_search_status_ul li').live('click', function () {
        afterOrderData.status = $(this).index();
        getAfterOrderList();
    });

    //高级搜索 控制搜索条数
    function venAfterOrderSearchNum() {
        $.ajax({
            url: SERVER_URL + '/afterorder/list',
            type: 'GET',
            data: {
                token: token,
                datatype: afterOrderData.datatype
            },
            success: function (e) {
                // 将返回值转换为json对象
                var oE = eval("(" + e + ")");
    ...
    $('.tanceng .btn_cancel').live('click', function () {
        $('.tanceng .dialog_screen').trigger('click');
    });
    //查看售后商品详单
    /*$('.ven_after_order_typea_look_product_info').die('click').live('click', function () {
     $.ajax({
     url: SERVER_URL + '/afterorder/info',
     type: 'GET',
     data: {
    ...
    //选择售后类型
    $('.ven_after_order_look_record_choose_type_ul li').live('click', function () {
        afterOrderLookRecordlistData.service_type = $(this).index();
        getAfterOrderRecordFn();
    });
    //选择售后时间
    $('.ven_after_order_look_record_choose_time li:first-of-type').live('click', function () {
        afterOrderLookRecordlistData.starttime = '';
        afterOrderLookRecordlistData.endtime = '';
        getAfterOrderRecordFn()
    });
    //选择售后时间 - 自定义时间
    $('.tanceng .ven_after_order_look_record_zdysj_save').live('click', function () {
    ...
    //添加评论
    $('.after_order_look_record_add_commit_save').live('click', function () {
    ...
    //作废操作
    $('.ven_after_order_invalid_btn').live('click', function () {
        venAfterOrderCurrentId = $(this).closest('tr').attr('afterorderid');
        $.ajax({
    ...
    //查看- 作废操作
    $('.ven_after_order_look_invalid_btn').live('click', function () {
        $.ajax({
            url: SERVER_URL + '/afterorder/setstatus',
            type: 'POST',
            data: {
    ...
    //查看 - 编辑操作
    $('.ven_after_order_look_edit_btn').live('click', function () {
        $('.right_sidebar_h').trigger('click');
    })

    //确认接收操作
    $('.ven_after_order_receive_btn').live('click', function () {
        venAfterOrderCurrentId = $(this).closest('tr').attr('afterorderid');
        $.ajax({
    ...
    //查看 - 接收接收操作
    $('.ven_after_order_look_receive_btn').live('click', function () {
        $.ajax({
            url: SERVER_URL + '/afterorder/opstatus',
            type: 'GET',
            data: {
    ...
    // 分配售后人员
    $('.ven_after_order_look_fpshry').live('click', function () {
        ...
        //选择负责人确认
        $('.tanceng .after_order_create_choose_owner_save').live('click', function () {
            $.ajax({
    ...
    // 新建售后单
    $('#ven_after_order_create_btn').live('click', function () {
        //创建时间
    ...
    //选择销售订单
    $('.tanceng .ven_after_order_create_choose_sell_order_btn').live('click', function () {
                ...
                if (oE.code == 0) {
                    ...
                    $.each(datalist, function (i, v) {
                        sellOrderHtml += '<tr sellorderid="' + v['id'] + '" vensellordercustomid="' + v['customer_id'] + '" vensellordercontacts="' + v['contacts_name'] + '" vensellordertel="' + v['contacts_tel'] + '" vensellorderlinkquoteid="' + v['quote_id'] + '">\
                            ...
                            <td>' + (v['dept_name']) + '</td>\
                            <td>' + (v['owner_name']) + '</td>\
                            <td>' + (v['delivery_at'].split(' ')[0]) + '</td>\
                            <td>' + (v['totals_product']) + '</td>\
                            <td>' + (v['tax_money_sum']) + '</td>\
                            <td>' + (v['totals']) + '</td>\
                            <td>' + (v['is_pay']) + '</td>\
                            <td>' + (v['is_ticket']) + '</td>\
                        </tr>';
    ...
    //搜索关键字
    $('.tanceng .ven_after_order_create_sell_order_searKey_btn').live('click', function () {
    ...
    $('.tanceng .ven_after_order_create_choose_sell_order_save').live('click', function () {
        ...
        afterOrderCreateData.customer_id = chosenOrder.attr('vensellordercustomid');
        afterOrderCreateData.contacts = chosenOrder.attr('vensellordercontacts');
        afterOrderCreateData.tel = chosenOrder.attr('vensellordertel');
        afterOrderLinkQuoteid = chosenOrder.attr('vensellorderlinkquoteid');
        $('.tanceng .ven_after_order_link_order_code_inp').val(chosenOrder.find('td').eq(1).html());
        $('.tanceng .ven_after_order_create_custom_name').val(chosenOrder.find('td').eq(2).html());
        $('.tanceng .ven_after_order_create_contacts').val(chosenOrder.attr('vensellordercontacts'));
        $('.tanceng .ven_after_order_create_tel').val(chosenOrder.attr('vensellordertel'));
        //编辑
    ...
    //新建售后单 - 选择商品保存
    $('.after_order_choose_product_save').live('click', function () {
        //普通商品
        ...
        $.each($('.tanceng .after_order_link_quote_goods_list tr'), function (i, v) {
            if ($('.tanceng .after_order_link_quote_goods_list tr').eq(i).find('input:checkbox').attr('checked') == 'checked') {
                ...
                //数量控制
                $('.tanceng .after_order_create_goods_num_change_btn').live('click', function () {
        ...
        $('.tanceng .after_order_create_package_parent_num_change').live('click', function () {
        ...
        $('.tanceng .after_order_create_package_child_num_change').live('click', function () {
        ...
        $('.tanceng .after_order_create_setting_parent_num_change').live('click', function () {
        ...
        $('.tanceng .after_order_create_setting_child_num_change').live('click', function () {
    ...
    //新建销售单 - 删除单条商品
    $('.tanceng .after_order_product_del_btn').live('click', function () {
        $(this).closest('tr').remove();
    });

    //新建售后单 - 删除套餐子商品
    $('.tanceng .after_order_create_package_child_chosen_list .after_order_product_del_btn').live('click', function () {
        var total = 0;
    ...
    //新建售后单 - 删除配置子商品
    $('.tanceng .after_order_create_setting_child_chosen_list .after_order_product_del_btn').live('click', function () {
        var total = 0;
    ...
    $('.tanceng .after_order_create_goods_box_del_btn').live('click', function () {
        afterOrderProductDelIndex = 0;
    });
    $('.tanceng .after_order_create_package_box_del_btn').live('click', function () {
        afterOrderProductDelIndex = 1;
    });
    $('.tanceng .after_order_create_setting_box_del_btn').live('click', function () {
        afterOrderProductDelIndex = 2;
    });
    $('.tanceng .xs_shd_delete').live('click', function () {
    ...
    //选择负责人
    $('.tanceng .ven_after_order_create_choose_owner_btn').live('click', function () {
        afterOrderChooseOwnerFn();
        //选择负责人
        $('.tanceng .person_left_nav').live('click', function () {
            //新建
        ...
        $('.tanceng .after_order_create_choose_owner_save').die('click').live('click', function () {
            $('.tanceng .after_order_create_owner_inp').val($('.tanceng .list_check').closest('li').find('.list_msg').html());
            $(this).closest('.dialog_box').remove();
        })
    });
    //负责人列表
    ...
    //新建售后提交
    $('.tanceng .after_order_create_submit').live('click', function () {
        //售后单编号
    ...
    //编辑售后
    $('.ven_after_order_edit_btn').live('click', function () {
        venAfterOrderCurrentId = $(this).closest('tr').attr('afterorderid');
    ...
    //开启自动分配
    $('.tanceng .after_order_setting_checkbox').live('click', function () {
        if ($(this).attr('checked') == 'checked') {
    ...
    //设置提交
    $('.tanceng .after_order_setting_submit').live('click', function () {
    ...
    //添加售后记录
    $('#ven_after_order_record_add_btn').live('click', function () {
        $.ajax({
            url: SERVER_URL + '/afterorder/info',
            type: 'GET',
            data: {
    ...
    //添加售后记录 - 提交
    $('.tanceng .ven_after_order_record_add_submit').live('click', function(){
        console.log()
    })
