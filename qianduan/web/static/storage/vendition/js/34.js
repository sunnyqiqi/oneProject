...
$(function () {
    ...
    /*if($.cookie('userinfo')){
        ...
        alert('请重新登录');
        window.location.href = "http://192.168.0.167:9094/web/index.html";//location.href实现客户端页面的跳转
    }*/
    /*var userinfo = eval('(' + $.cookie('userinfo') + ')');
    ...
    $('#ven_sell_chance_getall').live('click', function () {
        sellChanceData.page = 1;
        getSellChanceList('/customer-chance/list');
    });
    //团队销售机会
    $('#ven_sell_chance_getteamlist').live('click', function () {
        sellChanceData.page = 1;
        getSellChanceList('/customer-chance/teamlist');
    });
    //我的销售机会
    $('#ven_sell_chance_getminelist').live('click', function () {
        sellChanceData.page = 1;
        getSellChanceList('/customer-chance/mylist');
    });
    //抄送我的销售机会
    $('#ven_sell_chance_getcopylist').live('click', function () {
        sellChanceData.page = 1;
        getSellChanceList('/customer-chance/copylist');
    });
    // 获取销售机会列表
    function getSellChanceList(url) {
        venSellChanceSearch(url);
        $.ajax({
            url: SERVER_URL + url,
            type: 'GET',
            data: sellChanceData,
                ...
                if (oE.code == 0) {
                    ...
                    $.each(datalist, function (i, v) {
                        ...
                        if (v['is_invalid'] == 1) {
                            ...
                            //成交状态
                            statusClass = 'grey';
                            v['flow_status_name'] = '已作废';
                        } else {
                            sellChanceSortClass = '';
                            sellChanceSort = l_dbl(i + 1);
                            sellChanceOperateBtn = '<button class="but_cancel but_mix val_dialogTop but_exit ven_sell_chance_edit" name="vendition_xsjh_exit">编辑</button><button class="but_mix but_void but_r ven_sell_chance_invalid_btn">作废</button>';
                            //判断成交状态
                            if (v['status'] == 0) {
                                statusClass = 'c_y';
                            } else if (v['status'] == 1) {
                        ...
                        sellChanceHtml += '<tr sellchanceuid="' + v['uid'] + '" sellchanceid="' + v['id'] + '" class="' + sellChanceSortClass + '">\
                            ...
                            <td><span class="' + statusClass + '">' + (v['flow_status_name']) + '</span></td>\
                            <td>' + (v['step_name']) + '</td>\
                            <td>' + (v['track_num']) + '</td>\
                            ...
                            <td><button class="but_cancel but_mix r_sidebar_btn but_look ven_sell_chance_look" name="vendition_xsjh_rights">查看</button>' + sellChanceOperateBtn + '</td>\
                        </tr>';
                    });
                    //销售机会数据渲染
    ...
    //刷新列表
    $('#ven_sell_chance_refresh').live('click', function () {
        sellChanceData = {
            token: token,
            page: 1, // 页面
            num: 10, // 每页条数
        ...
        $('#ven_sell_chance_search_dept_inp').val('应用部门').css('color', '#CCCCCC');
        $('#ven_sell_chance_search_uname_inp').val('创建人').css('color', '#CCCCCC');
        $('#ven_sell_chance_search_updataname_inp').val('负责人').css('color', '#CCCCCC');
    ...
    //搜索关键字
    $('#ven_sell_chance_searKey_btn').live('click', function () {
        if ($('#ven_sell_chance_searKey').val() == '搜索销售机会') {
            alert('请输入搜索关键字');
        } else {
            sellChanceData.key = $('#ven_sell_chance_searKey').val();
            getSellChanceList($('#ven_sell_chance_tab .tabhover').attr('needurl'));
        }
    });

    //高级搜索 销售机会 控制下拉框
    function venSellChanceSearch(url) {
        $.ajax({
            url: SERVER_URL + url,
            type: 'GET',
            data: {
            ...
            success: function (e) {
                ...
                if (oE.code == 0) {
                    ...
                    //高级搜索 跟进人 数组
                    var sSearchOwnerId = '';
                    var sSearchOwnerName = '';
                    //高级搜索 创建人 数组
                    var searchUnameArr = [];
                    //高级搜索遍历
                    $.each(datalist, function (i, v) {
                        sSearchDeptId += v['dept'] + ',';
                        sSearchDeptName += v['dept_name'] + '、';
                        //提取创建人数组
                        searchUnameArr.push({uid: v['uid'], uname: v['uname']});
                        //提取跟进人数组
                        sSearchOwnerId += v['owner'] + ',';
                        sSearchOwnerName += v['owner_name'] + '、';
                    });
                    // 应用部门id和name放在一个数组里
                    ...
                    //高级搜索 应用部门
                    $('#ven_sell_chance_search_dept_ul').html(searchDeptUl);

                    //高级搜索 创建人
                    $.each(getJsonArr(searchUnameArr), function (i, v) {
                        searchUnameUl += '<li searchUid="' + v['uid'] + '">' + v['uname'] + '</li>'
                    });
                    $('#ven_sell_chance_search_uname_ul').html(searchUnameUl);
                    //高级搜索 跟进人
                    var aSearchOwner = getJsonArr(getDataArr(sSearchOwnerId.substr(0, sSearchOwnerId.length - 1), sSearchOwnerName.substr(0, sSearchOwnerName.length - 1)));
                    var searchOwnerArr = [];
                    $.each(aSearchOwner, function (i, v) {
                        searchOwnerUl += '<li owner="' + v['title'] + '">' + v['val'] + '</li>'
                    });
                    $('#ven_sell_chance_search_owner_ul').html(searchOwnerUl);
                }
            }
        })
    }
    ...
    // 搜索应用部门
    $('#ven_sell_chance_search_dept_ul li').live('click', function () {
        sellChanceData.dept = $(this).attr('searchdeptid');
        console.log(sellChanceData.dept);
        getSellChanceList($('#ven_sell_chance_tab .tabhover').attr('needurl'));
    });
    //搜索创建人
    $('#ven_sell_chance_search_uname_ul li').live('click', function () {
        sellChanceData.uid = $(this).attr('searchUid');
        getSellChanceList($('#ven_sell_chance_tab .tabhover').attr('needurl'));
    });
    //搜索跟进人
    $('#ven_sell_chance_search_owner_ul li').live('click', function () {
        sellChanceData.owner = $(this).attr('owner');
        getSellChanceList($('#ven_sell_chance_tab .tabhover').attr('needurl'));
    });
    //搜索 状态
    $('#ven_sell_chance_search_status_ul li').live('click', function () {
    ...
    //作废操作
    $('.ven_sell_chance_invalid_btn').live('click', function () {
    ...
    var sellChanceId = '';
    var sellChanceUid = '';
    $('.lik_sell_chance_btn_del').live('click', function () {
        sellChanceId = $(this).closest('tr').attr('sellchanceid');
        $('#ven_sell_chance_del_inp').val(sellChanceId);
    });
    $('.ven_sell_chance_del_btn_submit').live('click', function () {
        $.ajax({
    ...
    //查看操作
    $('.ven_sell_chance_look').live('click', function () {
        //默认显示基本信息
                ...
                if (oE.code == 0) {
                    ...
                    if(data.customfields != ''){
                        ...
                        $.each(data.customfields, function (i, v) {
                            venSellChanceFieldHtml += '<p class="l-s-x">' + v['title'] + '：<span>' + v['val'] + '</span></p>'
                        });
                ...
                if (oE.code == 0) {
                    ...
                    $.each(data, function (i, v) {
                        ...
                        venSellChanceLookFlow += '<li>\
                                                        ...
                                                        <span class="sale_saleflow_circleP" style="text-indent:10px;">' + v['name'] + '</span>\
                        ...
                        $.each(v['track'], function (i2, v2) {
                            ...
                            $.each(v2['commit_log'], function (i3, v3) {
                                venSellChanceLookTrackCommits += '<div class="work_rsp_text rsp_text">' +
                                    '<img class="tx" src="' + v3['face'] + '" style="margin-top: 4px;">' +
                            ...
                            <span class="left m_top_5 m_left_5" style="background: url(' + v2['face'] + ') no-repeat;background-size:100% 100%;"></span>\
                        ...
                        <div class="work_report_upload vendition_khbf2">\
                            <img src="' + v2['imgurl'] + '" alt="工作计划" class="val_dialog" name="vendition_khbf_imglist" style="border-radius: 3px;">\
                            </div>\
                    ...
                    //跟进反馈、进入下一阶段、完成跟进  按钮
                    $('#ven_sell_chance_look_track_flow_ul li .sale_saleflow_circle').live('click', function () {
                    ...
                    //添加跟进反馈
                    $('.tanceng .ven_sell_chance_addtrack_submit').live('click', function () {
    ...
    $('.ven_sell_chance_look_track_del').live('click', function () {
        venSellChanceLookTrackId = $(this).closest('.ven_sell_chance_look_track_box').attr('trackid');
    });
    $('.tanceng .btn_scgjjl_submit').live('click', function () {
        $.ajax({
    ...
    //添加评论
    $('.sell_chance_look_track_commit_add_submit').live('click', function () {
        venSellChanceLookTrackId = $(this).closest('.ven_sell_chance_look_track_box').attr('trackid');
    ...
    //新建销售机会
    $('#ven_sell_chance_create_btn').live('click', function () {
                ...
                if (oE.code == 0) {
                    ...
                    $.each(datalist, function (i, v) {
                        venSellChanceFieldHtml += '<div class="t_textinput">\
                            ...
                            <input type="text" class="time_input" value="" style="background: #fff;border:1px solid #e6e6e6;">\
                            </div>\
                            </div>'
                    });
    ...
    //选择客户
    $('.tanceng .ven_sell_chance_choose_customer').live('click', function () {
        getCusListSort();
        getCusList();
        xs_kh_gjss_xlk();
    });
...
//					$('.hr_left_all_l_n').html(datalist.length)
                ...
                if (e.code == 0) {
                    ...
                    for (var i = 0; i < cuslist.length; i++) {
                        ...
                        var cusSort = '',
                            cusBtnClass = '',
                            cusContactClass = '';
                        oCuslist += '<tr class="' + cusSta + '" cusId="' + cuslist[i].id + '"><td><input type="radio" name="222" ' + (i == 0 ? 'checked' : '') + '></td><td>' + l_dbl(i + 1) + '</td><td>' + cuslist[i].code_sn + '</td><td>' + cuslist[i].name + '</td><td>' + cuslist[i].tel + '</td><td><div class="text_line" style="width: 9em;"><span class="ellipsis">' + cuslist[i].address + '</span></div></td><td>' + cuslist[i].industry_big_name + '</td><td>' + cuslist[i].grade_name + '</td><td class="f_color ' + cusContactClass + '">' + con1 + '<div class="vent_client_contMsgBox" style="display: none;z-index:99;"><i class="vent_client_torr"></i>' + con2 + '</div></td><td>' + cuslist[i].introducer_name + '</td><td>' + cuslist[i].note + '</td></tr>'
                    }
    ...
    //选择分类获取客户
    $('.tanceng .ven_sell_chance_create_choose_customer_sort li').live('click', function () {
    ...
    //选择客户
    $('.tanceng .ven_sell_chance_create_choose_customer_submit').live('click', function () {
        ...
        //新建销售机会用
        $('.tanceng .sell_chance_create_customer_chosen').val(sellChanceCusChosen['val']);
    ...
    // 选择销售阶段
    $('.tanceng .ven_sell_chance_create_choose_flow_btn').live('click', function () {
        getSellFlowList();
        venSellFlowSearch();
    });
    // 定义销售流程参数
                ...
                if (oE.code == 0) {
                    ...
                    $.each(datalist, function (i, v) {
                        ...
                        if (datalist[i]['status'] == 0) {
                            statusClass = 'c_g';
                            sBtn = '<button class="but_mix but_exit val_dialogTop ven_sell_flow_edit" name="vendition_addxslc_exit">编辑</button><button class="but_cancel but_mix but_r lik_sell_flow_btn_ty">停用</button>'
                        } else {
                            statusClass = 'c_r';
    ...
    //搜索关键字
    $('.tanceng .ven_sell_chance_create_choose_flow_searKey_btn').live('click', function () {
    ...
    //搜索状态
    $('.tanceng .ven_sell_chance_create_choose_flow_search_status_ul li').live('click', function () {
        if ($(this).index() == 0) {
            sellFlowData.status = '';
    ...
    //搜索部门
    $('.tanceng .ven_sell_chance_create_choose_flow_search_dept_ul li').live('click', function () {
        sellFlowData.dept = $(this).attr('searchdeptid');
        getSellFlowList();
    });
    //选择流程确认
    $('.tanceng .ven_sell_chance_create_choose_flow_submit').live('click', function () {
        ...
        //新建用
        $('.tanceng .ven_sell_chance_create_choose_flow_inp').val(venSellChanceCreateChooseFlow['val']);
    ...
    /* // 选择负责部门
     $('.tanceng .ven_sell_chance_create_choose_dept_btn').die('click').live('click', function () {
     ...
     //选择部门左侧选择
     $('.tanceng .ven_sell_chance_create_choose_dept_list ul .deptChild').die('click').live('click', function () {
     ...
     //新建>选择部门确认
     $('.tanceng .ven_sell_chance_create_choose_dept_save').die('click').live('click', function () {
    ...
    //选择跟进人
    $('.tanceng .ven_sell_chance_create_choose_owner').live('click', function () {
    ...
    //添加抄送人
    $('.tanceng .ven_sell_chance_create_choose_copy_btn').live('click', function () {
            ...
            success: function (e) {
                ...
                if (oE.code == 0) {
                    ...
                    $('.tanceng .ven_sell_chance_choose_copy_save').die('click').live('click', function () {
                        ...
                        $('.tanceng .ven_sell_chance_create_add_copy_list').html('<li><em class="icon_personBtn icon_personBtn_msg"><i class="icon_personNext"></i></em> <em class="icon_adderBtn"></em> <p class="box_adderName">' + uname + '</p> </li> <li> <em class="icon_personBtn icon_personBtn_add val_dialogTop ven_sell_chance_create_choose_copy_btn" name="xs_xsjh_csg"></em> </li>').prepend(copyChosen);
                        $('.tanceng .ven_sell_chance_edit_copy_list').html('<li><em class="icon_personBtn icon_personBtn_msg"><i class="icon_personNext"></i></em> <em class="icon_adderBtn"></em> <p class="box_adderName">' + uname + '</p> </li> <li> <em class="icon_personBtn icon_personBtn_add val_dialogTop ven_sell_chance_create_choose_copy_btn" name="xs_xsjh_csg"></em> </li>').prepend(copyChosen);
                        $(this).closest('.dialog_box').remove()
                    })
                }
            },
    ...
    //新建提交
    $('.tanceng .ven_sell_chance_create_submit').live('click', function () {
         ...
         return false;
            ...
            $.ajax({
                ...
                success: function (e) {
                    ...
                    if (oE.code == 0) {
                        alert('添加成功');
                        $('.tanceng .ven_sell_chance_create_submit').closest('.dislog_box').remove();
                        $('.tanceng').css('display', 'none');
                        getSellChanceList($('#ven_sell_chance_tab .tabhover').attr('needurl'));
                    } else {
                        alert(oE.msg)
    ...
    //编辑销售机会
    $('.ven_sell_chance_edit').live('click', function () {
        sellChanceId = $(this).closest('tr').attr('sellchanceid');
        ...
        $.ajax({
            ...
            success: function (e) {
                ...
                if (oE.code == 0) {
                    ...
                    // 销售阶段
                    $('.tanceng .ven_sell_chance_edit_flow').val(data['step_name']);
                    ...
                    if (data['copy_list'] && data['copy_name']) {
                        var editCopy = getDataArr(data['copy_list'], data['copy_name']);
                        var sEditCopy = '';
                        $.each(editCopy, function (i, v) {
                            sEditCopy += '<li userinfoid="' + v['title'] + '">\
                                <i class="del_img_1">-</i><em class="icon_personBtn icon_personBtn_msg"><i class="icon_personNext"></i></em>\
                                <em class="icon_adderBtn"></em>\
                                <p class="box_adderName">' + v['val'] + '</p>\
                                </li>'
                        });
                    ...
                    if(data.customfields != ''){
                        var venSellChanceFieldHtml = '';
                        $.each(data.customfields, function (i, v) {
                            venSellChanceFieldHtml += '<div class="t_textinput"><div class="t_left">'+v['title']+'</div><div class="t_right clearfix"><input type="text" class="time_input" value="'+v['val']+'" style="background: #fff;border:1px solid #e6e6e6;"></div></div>'
                        });
    ...
    //编辑销售机会 选择客户
    $('.tanceng .ven_sell_chance_edit_choose_customer').live('click', function () {
        getCusListSort();
        getCusList();
        xs_kh_gjss_xlk();
    });
    //选择销售流程
    $('.tanceng .ven_sell_chance_edit_choose_flow').live('click', function () {
        getSellFlowList();
        venSellFlowSearch();
    });
    //选择负责部门
    $('.tanceng .ven_sell_chance_edit_choose_dept_btn').live('click', function () {
        venPayFlowChooseDept();
    });
    //选择跟进人
    $('.tanceng .ven_sell_chance_edit_choose_owner').live('click', function () {
        venSellChanceChooseOwner();
    });
    //选择抄送人
    $('.tanceng .ven_sell_chance_edit_choose_copy').live('click', function () {
        venSellChanceChooseCopy()
    });
    //编辑销售机会提交
    $('.tanceng .ven_sell_chance_edit_submit').live('click', function () {
    ...
    //查看  编辑
    $('#ven_sell_chance_look_edit_btn').live('click', function () {
        getSellChanceInfo(sellChanceId);
        /*venSellChanceEditData = {
         token: token,
         chance_id: sellChanceId, // 销售机会id
    ...
    //查看  作废
    $('.ven_sell_chance_look_invalid_btn').live('click', function () {
        $.ajax({
            url: SERVER_URL + '/customer-chance/setstatus',
            type: 'POST',
            data: {
    ...
    //查看  删除
    $('#ven_sell_chance_look_del_btn').live('click', function () {
        $('#ven_sell_chance_del_inp').val(sellChanceId);
    });
    // 查看  更多
    $('#ven_sell_chance_look_more').find('li').live('click', function () {
        $('.slider_head_More').trigger('click');
        $('.slider_head_close').trigger('click');
    })
    // 查看  更改跟进人
    $('#ven_sell_chance_look_change_owner_btn').live('click', function () {
        venSellChanceChooseOwner();
        $('.tanceng .ven_sell_chance_look_change_owner_save').live('click', function () {
            var changeOwnerList = '';
            $.each($('.tanceng .ven_sell_chance_create_owner_chosen_list li'), function (i, v) {
    ...
    //设置自定义字段
    $('#ven_sell_chance_setting_btn').live('click', function () {
        $.ajax({
            url: SERVER_URL + '/customer/settinglist',
            type: 'GET',
            data: {
                ...
                if (oE.code == 0) {
                    ...
                    $.each(datalist, function (i, v) {
                        venSellChanceFieldHtml += '<div class="t_textinput">\
                            ...
                            <span class="t_right_span  page34_sxjhzdyDelete" style="cursor: pointer;background: #FF6C60;">删除</span>\
                            </div>\
                            </div>'
                    });
    ...
    //设置自定义字段 - 保存
    $('.tanceng .ven_sell_chance_setting_submit').live('click', function () {
        var venSellChanceField = [];
        $.each($('.tanceng .ven_sell_chance_setting_oldlist .t_textinput'), function (i, v) {
            venSellChanceField.push($('.tanceng .ven_sell_chance_setting_oldlist .t_textinput').eq(i).find('.t_left').text());
        });
        $.each($('.tanceng .ven_sell_chance_setting_newlist .ven_sell_chance_setting_newlist_field_inp'), function (i, v) {
            if ($('.tanceng .ven_sell_chance_setting_newlist .ven_sell_chance_setting_newlist_field_inp').eq(i).val() != 'QQ号') {
                venSellChanceField.push($('.tanceng .ven_sell_chance_setting_newlist .ven_sell_chance_setting_newlist_field_inp').eq(i).val());
            }
        });
        $.ajax({
